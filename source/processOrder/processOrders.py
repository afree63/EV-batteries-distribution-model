...
class orderProcessing(object):
    ...
    def populateData(self):
        ...
        for i in range(self.number_of_medians):                         # O(number of medians * car in hub)
            ...
            for index in range(self.number_of_cars):
                self.data_of_car[i][index] = (medians.iloc[i].node_id.astype('int64'), 0, 0, 0)        #(current location of car, time upto which committed, battery in car, previous location)
        ...
        for i in range(self.number_of_solar_farm):                      # O(number of farm * car in farm)
            ...
            for index in range(self.number_of_solar_farm_cars):
                self.farm_car_data[i][index] = (self.solar_farm_nodes[i], 0, 0, 0)        #(current location of car, time upto which committed, battery in car, supply track id)
    ...
    def requestFarmBattery(self, time, cluster_number):
        ...
        if self.battery_in_hub[cluster_number] + self.en_route[cluster_number] < self.hourToCover * lastHourOrders:
            ...
            if farm_index != -1:
                ...
                self.farm_car_data[farm_index][car_index] = (medians.iloc[cluster_number].node_id.astype('int64'), minimum_time + 10*60, battery + self.hourToCover * lastHourOrders, self.supplyToHubId)

                if delivery_time not in self.reload_data:
                    self.reload_data[delivery_time] = []

                self.reload_data[delivery_time].append((cluster_number, self.hourToCover * lastHourOrders, farm_index, car_index, self.supplyToHubId))

                self.hubData.append((cluster_number, second_to_time(time), self.hourToCover * lastHourOrders, self.battery_in_hub[cluster_number], self.en_route[cluster_number], farm_index, self.number_of_medians*self.number_of_cars + farm_index*self.number_of_solar_farm_cars + car_index, second_to_time(delivery_time)))
    ...
    def batteryToFarm(self, time):
        if time in self.car_reach_farm:
            for car_data in self.car_reach_farm[time]:                                              # O(number of battery request)
                ...
                self.farm_car_data[farm_index][car_index] = (self.solar_farm_nodes[farm_index], 0, 0, 0)
    ...
    def carToHub(self, time, car_index, cluster_number):
        ...
        self.data_of_car[cluster_number][car_index] = (medians.iloc[cluster_number].node_id.astype('int64'), time_when_reach + 20*60, 0, last_time)

        self.carData.append((cluster_number, cluster_number*self.number_of_cars + car_index, second_to_time(time_when_reach), second_to_time(time_when_reach - initialTime, True)))
    ...
    def processOrder(self, time):
        if time in self.time_to_order:
            ...
            for item in order_nodes:   
                self.order_count += 1
                order_node = item[0]                                                                       # O (number of orders * number of hub cars * nlogn), n = number of nodes
                waiting_time = item[1]
                ...
                for car in self.data_of_car[cluster_number].keys():                                                 # O (number of hub cars * nlogn), n = number of nodes
                    ...
                    if battery == 0:
                        if carStartingFromHubIndex == -1:
                            carStartingFromHubIndex = car
                ...
                if self.battery_in_hub[cluster_number] > 0 and carStartingFromHubIndex != -1:
                    location = self.data_of_car[cluster_number][carStartingFromHubIndex][0]
                    last_time = max(time, self.data_of_car[cluster_number][carStartingFromHubIndex][1])
                    hour = last_time // 3600
                    ...
                    if time_when_reach <= time + waiting_time and time_when_reach < minimum_time:
                        minimum_time = time_when_reach
                        car_index = carStartingFromHubIndex
                ...
                if car_index != -1:
                    ...
                    if battery == 0:
                        battery = min(self.hub_car_capacity, self.battery_in_hub[cluster_number])
                    ...
                    self.confirmOrderData.append((order_node, cluster_number, second_to_time(time), second_to_time(waiting_time, True), second_to_time(minimum_time), second_to_time(minimum_time - last_time, True), location, cluster_number*self.number_of_cars + car_index, battery))
                else:
                    self.total_rejected += 1
                    # print("X X X X X X X X X X X --- Order rejected -> cluster number:", cluster_number, "--- X X X X X X X X X X X")
                    reason = "Time constraint"
                    if one_with_battery == False and self.battery_in_hub[cluster_number] == 0:
